package ch.skyfy.customendermanbehavior.commands

import ch.skyfy.customendermanbehavior.CustomEndermanBehaviorMod
...
class ReloadFilesCmd : Command<ServerCommandSource> {
    ...
    companion object {
            ...
            val cmd = CommandManager.literal("bypass-anvil-restriction").requires { source -> source.hasPermissionLevel(4) }
                .then(
                    CommandManager.literal("reload").then(
                        CommandManager.argument("fileName", StringArgumentType.string()).suggests(Companion::getConfigFiles).executes(ReloadFilesCmd())
                    )
                )
        ...
        private fun <S : ServerCommandSource> getConfigFiles(commandContext: CommandContext<S>, suggestionsBuilder: SuggestionsBuilder): CompletableFuture<Suggestions> {
            ...
            list.add(Configs.ENDERMAN_CONFIG.relativeFilePath.fileName.toString())
    ...
    override fun run(context: CommandContext<ServerCommandSource>): Int {
        ...
        when (fileName) {
            "config.toml" -> list.add(ConfigManager.reloadConfig(Configs.ENDERMAN_CONFIG))
        }

        if (list.contains(false)) {
            context.source.sendFeedback(Text.literal("Configuration could not be reloaded"), false)
            CustomEndermanBehaviorMod.LOGGER.warn("Configuration could not be reloaded")
        } else {
            context.source.sendFeedback(Text.literal("The configuration was successfully reloaded"), false)
            CustomEndermanBehaviorMod.LOGGER.info("The configuration was successfully reloaded")
        }
